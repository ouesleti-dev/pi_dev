...
import com.example.javaproject.Services.BadWordsAPIService;
import com.example.javaproject.Services.SMSService;
...
public class AjouterReclamation {
    ...
    private void handleAjouterButton(ActionEvent event) {
        ...
        reclamation.setIdUtilisateur(2); // À remplacer par l'ID utilisateur réel
        ...
        try (Connection connection = Myconnection.getInstance().getCnx()) {
            String sql = "INSERT INTO reclamation (idutilisateur, titre, message, typereclamation, statut, datereclamation, priorite, categorie) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    private void sendAdminNotification(Reclamation reclamation) {
        new Thread(() -> {
            try {
                new SMSService().sendAdminNotification("21656521654"); // Numéro admin
            } catch (Exception e) {
    ...
    private boolean validateTextField(TextInputControl field, Label errorLabel,
        ...
        try {
            if (badWordsAPIService.containsProfanity(text)) {
